...
exports.cover = function(req,res,filePath,type,callback){
    console.log(filePath);
    var fileName = filePath.split("\\");
    fileName = fileName[fileName.length-1];
    console.log(fileName);
    var target_path = '&lt; 我本地存图片的路径 &gt;' + fileName;
    ...
    fs.readFile(target_path, function(err, data){
        console.log("data length is " + data.length);
        qiniu.io.put(uptoken, 'image_'+type+"_"+time+"_"+ fileName, data, extra, function(err, ret) {
            fs.unlinkSync(target_path);
            console.log(ret);
            var url = 'http://7xnd0a.com1.z0.glb.clouddn.com/'+ret.key;
            callback(err,url);
